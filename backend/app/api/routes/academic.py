...
def _create_training_teacher_report_workbook(report: dict[str, Any]) -> Workbook:
    ...
    late_headers = ['Giảng viên', 'Môn', 'Lớp', 'Mã SV', 'Username', 'Họ tên', 'Số Quiz chậm tiến độ', 'Danh sách Quiz chậm tiến độ', 'Điều kiện thi', 'Lý do']
    ...
    _setup_sheet(exam_ws, exam_headers, [26, 12, 16, 14, 20, 26, 20, 18, 90])
    for row in report.get('student_watch_rows') or []:
        if row.get('exam_status') != 'not_eligible':
            continue
        _append_row(exam_ws, [row.get('teacher_name'), row.get('subject_code'), row.get('class_code'), row.get('student_code'), row.get('student_username'), row.get('student_name'), row.get('exam_cutoff_date'), row.get('exam_status_label') or 'Không được thi', row.get('exam_reasons')])
    ...
    notes = [
        ...
        'SinhVienCanTheoDoi liệt kê sinh viên có trạng thái cần xử lý: chưa đồng bộ CMS, chưa enroll, chưa học, tiến độ thấp, điểm thấp, lỗi đồng bộ hoặc chậm các mốc tiến độ Quiz.',
    ...
    refresh_meta = report.get('learning_refresh') if isinstance(report.get('learning_refresh'), dict) else None
    if refresh_meta and str(refresh_meta.get('learning_platform') or '').lower() == 'cms':
        notes.insert(0, (
            'Dữ liệu điểm CMS/Open edX đã được làm mới trực tiếp trước khi xuất Excel: '
            f"{int(refresh_meta.get('refreshed_class_count') or 0)}/"
            f"{int(refresh_meta.get('mapped_class_count') or 0)} lớp đã map Course CMS được xác nhận mới; "
            f"{int(refresh_meta.get('failed_class_count') or 0)} lớp lỗi. "
            'Nếu không xác nhận được điểm mới, job xuất Excel sẽ thất bại thay vì dùng điểm snapshot cũ.'
        ))
...
def _enqueue_teacher_report_job(
    *,
    ...
    for active in active_query.order_by(AcademicTeacherReportJob.created_at.desc()).limit(20).all():
        ...
        if active_request.get('request_key') == request_key:
            return active
...
def enqueue_training_teacher_cache_job(
    ...
    return _enqueue_teacher_report_job(
        db=db,
        user=user,
...
@router.post('/training/classes/{class_id}/export/jobs', response_model=AcademicTeacherReportJobOut)
def enqueue_training_class_export_job(
    class_id: str,
    user: UserContext = Depends(_require_academic_view_permission),
    db: Session = Depends(get_db),
):
    """Export one authorized class with a forced fresh CMS grade pull.

    AP-assigned teachers can export their own class; admins/campus-scoped users
    keep the same class visibility enforced by AcademicService.
    """
    service = AcademicService(db)
    detail = service.get_class_detail(user, class_id)
    platform = str(detail.get('learning_platform') or 'cms').strip().lower()
    decision = service.access_decision(user)
    teacher_id = None
    if not decision.unrestricted and decision.teacher_ids:
        assigned = db.query(AcademicTeacherAssignment.teacher_id).filter(
            AcademicTeacherAssignment.class_id == class_id,
            AcademicTeacherAssignment.teacher_id.in_(sorted(decision.teacher_ids)),
        ).first()
        teacher_id = str(assigned[0]) if assigned else None
    return _enqueue_teacher_report_job(
        db=db,
        user=user,
        job_type='export_excel',
        term_id=str(detail.get('term_id') or ''),
        branch=detail.get('branch'),
        campus=detail.get('campus'),
        learning_platform='udemy' if platform == 'udemy' else 'cms',
        teacher_id=teacher_id,
        class_id=class_id,
    )
...
def download_training_teacher_report_job_file(
    ...
    try:
        if not storage.exists(job.file_path):
            raise HTTPException(status_code=404, detail='File báo cáo không còn tồn tại')
        stream = storage.iter_bytes(job.file_path)
    except StorageError as exc:
        raise HTTPException(status_code=503, detail='Kho lưu trữ báo cáo chưa sẵn sàng') from exc
    filename = job.file_name or 'teacher-management-report.xlsx'
...
async def preview_udemy_plan_import(
    ...
    try:
        ...
        raise public_http_exception(
            status_code=400,
            code='UDEMY_PLAN_IMPORT_INVALID',
            message='File kế hoạch Udemy không hợp lệ.',
            logger_name=__name__,
        ) from exc
...
async def create_udemy_progress_import_job(
    ...
    try:
        ...
        pass
    if not files:
        raise HTTPException(status_code=400, detail='Hãy chọn ít nhất một file Udemy .xlsx/.csv hoặc một gói .zip.')
    if len(files) > UdemyProgressService.MAX_FILES:
        raise HTTPException(status_code=400, detail=f'Mỗi lần tối đa {UdemyProgressService.MAX_FILES} file hoặc gói upload.')

    branch_value = str(branch or 'poly').strip().lower()
    if branch_value not in {'poly', 'ptcd'}:
        raise HTTPException(status_code=400, detail='Hệ chỉ nhận poly hoặc ptcd.')
    selected_term = db.get(AcademicTerm, term_id)
    selected_block = db.get(AcademicBlock, block_id)
    if not selected_term:
        raise HTTPException(status_code=400, detail='Không tìm thấy học kỳ đã chọn.')
    if not selected_block or selected_block.term_id != selected_term.id:
        raise HTTPException(status_code=400, detail='Block không thuộc học kỳ đã chọn.')

    service = UdemyProgressService(db)
    logical_files: list[dict[str, Any]] = []
    rejected_files: list[dict[str, Any]] = []
    compressed_upload_bytes = 0
    for upload in files:
        filename = safe_upload_filename(upload.filename or 'udemy-progress.xlsx')
        suffix = Path(filename).suffix.lower()
        read_limit = UdemyProgressService.MAX_TOTAL_UPLOAD_BYTES if suffix == '.zip' else UdemyProgressService.MAX_FILE_BYTES
        raw = await upload.read(read_limit + 1)
        compressed_upload_bytes += len(raw)
        if len(raw) > read_limit:
            limit_mb = max(1, read_limit // (1024 * 1024))
            raise HTTPException(status_code=400, detail=f'File {filename} vượt giới hạn upload {limit_mb} MB.')
        if compressed_upload_bytes > UdemyProgressService.MAX_TOTAL_UPLOAD_BYTES:
            raise HTTPException(
                status_code=400,
                detail=f'Tổng dung lượng upload vượt giới hạn {max(1, UdemyProgressService.MAX_TOTAL_UPLOAD_BYTES // (1024 * 1024))} MB.',
            )
        try:
            expanded, archive_rejections = service.expand_upload_payload(
                filename=filename,
        ...
        logical_files.extend(expanded)
        rejected_files.extend(archive_rejections)

    if len(logical_files) > UdemyProgressService.MAX_FILES:
        raise HTTPException(status_code=400, detail=f'Mỗi lần tối đa {UdemyProgressService.MAX_FILES} file Udemy sau khi giải nén ZIP.')
    if delivery_id and len(logical_files) != 1:
        raise HTTPException(status_code=400, detail='Import tại một môn chỉ nhận một file. Hãy dùng “Import hàng loạt Udemy” để tải nhiều file hoặc ZIP.')

    total_bytes = sum(len(item['raw']) for item in logical_files)
    if total_bytes > UdemyProgressService.MAX_TOTAL_UPLOAD_BYTES:
        raise HTTPException(
            status_code=400,
            detail=f'Tổng dữ liệu cần xử lý vượt giới hạn {max(1, UdemyProgressService.MAX_TOTAL_UPLOAD_BYTES // (1024 * 1024))} MB.',
        )

    expected_term_tokens = service.expected_term_tokens(selected_term)
    bulk_mode = len(logical_files) > 1 or any(item.get('source_archive') for item in logical_files)
    prepared: list[dict[str, Any]] = []

    def reject_file(item: dict[str, Any], *, code: str, message: str, identity: dict[str, Any] | None = None) -> None:
        ...
        rejected_files.append({
            ...
            'reason_code': code,
            'message': message,
            'detected_subject_code': identity.get('subject_code'),
            'detected_term_code': identity.get('term_code'),
        })

    for logical in logical_files:
        filename = safe_upload_filename(logical['filename'])
        raw = logical['raw']
        identity: dict[str, Any] = {}
        try:
            service.validate_upload_content(filename=filename, raw=raw)
            identity = service.inspect_report_identity(filename=filename, raw=raw)
            filename_subject = service.subject_code_from_filename(filename)
            report_subject = str(identity.get('subject_code') or '').strip().upper() or None
            report_term = str(identity.get('term_code') or '').strip().upper() or None
            report_branch = str(identity.get('branch') or '').strip().lower() or None

            if filename_subject and report_subject and filename_subject != report_subject:
                raise ValueError(
                    f'Tên file nhận diện môn {filename_subject} nhưng Tiêu đề lộ trình bên trong là {report_subject}. Không import để tránh ghi nhầm môn.'
                )
            if report_branch and report_branch != branch_value:
                raise ValueError(
                    f'Báo cáo thuộc hệ {report_branch.upper()} nhưng đang import vào hệ {branch_value.upper()}.'
                )
            if report_term and expected_term_tokens and report_term not in expected_term_tokens:
                expected_label = ', '.join(sorted(expected_term_tokens))
                raise ValueError(
                    f'Báo cáo thuộc học kỳ {report_term}, không khớp học kỳ đã chọn {selected_term.term_name} ({expected_label}).'
                )
            ...
            delivery, subject = service.resolve_delivery(
                ...
                subject_code_hint=report_subject or filename_subject,
            )
            expected_subject = str(report_subject or filename_subject or '').upper()
            if expected_subject and str(subject.subject_code or '').upper() != expected_subject:
                raise ValueError(
                    f'File nhận diện môn {expected_subject} nhưng phạm vi import đang trỏ tới {subject.subject_code}.'
                )
        except ValueError as exc:
            if not bulk_mode:
                raise HTTPException(status_code=400, detail=f'{filename}: {exc}') from exc
            ...
            if 'Tiêu đề lộ trình bên trong' in text_message:
                reason_code = 'SUBJECT_CODE_MISMATCH'
            elif 'học kỳ' in text_message and ('không khớp' in text_message or 'không thuộc' in text_message):
                reason_code = 'TERM_MISMATCH'
            elif 'hệ ' in text_message and 'đang import' in text_message:
                reason_code = 'BRANCH_MISMATCH'
            elif 'Không tìm thấy môn' in text_message or 'triển khai môn' in text_message:
                reason_code = 'DELIVERY_NOT_FOUND'
            elif 'chỉ môn Udemy' in text_message:
                reason_code = 'NOT_UDEMY'
            reject_file(logical, code=reason_code, message=text_message, identity=identity)
            continue
        ...
        prepared.append({
            ...
            'delivery': delivery,
            'subject': subject,
            'source_archive': logical.get('source_archive'),
            'report_identity': identity,
        })

    if not prepared:
        examples = '; '.join(f"{item.get('file_name')}: {item.get('message')}" for item in rejected_files[:3])
        suffix = f' Chi tiết: {examples}' if examples else ''
        raise HTTPException(status_code=400, detail=f'Không có file Udemy hợp lệ để import.{suffix}')
    ...
    try:
        for item in prepared:
            ...
            if is_duplicate:
                ...
                reference = storage.put_bytes(
                    f'udemy-progress-imports/{parent.id}/{batch.id}-{item["filename"]}',
                    item['raw'],
                    content_type=None,
                )
                created_storage_refs.append(reference)
                batch.file_path = reference
                db.add(batch)
        ...
        if not queued_ids:
            ...
            parent.progress_label = 'Không có file mới cần xử lý'
            parent.result_json = {
                'ok': True,
                'queued_count': 0,
        ...
        db.rollback()
        for reference in created_storage_refs:
            try:
    ...
    parts = [f'xếp hàng {len(queued_ids)} file']
    if duplicate_count:
        parts.append(f'bỏ qua {duplicate_count} file trùng')
    if rejected_files:
        parts.append(f'loại {len(rejected_files)} file không hợp lệ')
    message = 'Đã ' + '; '.join(parts) + '.'
    log_audit(
        db,
...
def retry_udemy_progress_import_batch(
    ...
    storage = get_object_storage()
    try:
        raw = storage.read_bytes(str(source.file_path or ''))
    except StorageError as exc:
        raise HTTPException(status_code=410, detail='File import gốc đã hết thời gian lưu. Hãy tải file lên lại.') from exc
    try:
        UdemyProgressService.validate_upload_content(filename=source.file_name, raw=raw)
    except ValueError as exc:
    ...
    try:
        retry_reference = storage.put_bytes(
            f'udemy-progress-imports/{parent.id}/{retry_batch.id}-{safe_upload_filename(source.file_name)}',
            raw,
        )
        retry_batch.file_path = retry_reference
        parent.request_json = json_safe_value({
            'term_id': delivery.term_id,
            'block_id': delivery.block_id,
            'branch': str(delivery.branch or 'poly').lower(),
        ...
        if retry_reference:
            try:
                storage.delete(retry_reference, missing_ok=True)
            except Exception:
                pass
        raise
...
def download_udemy_progress_import_errors(
    ...
    storage = get_object_storage()
    try:
        if not storage.exists(batch.error_report_path):
            raise HTTPException(status_code=404, detail='File lỗi đã hết hạn hoặc không còn tồn tại.')
        stream = storage.iter_bytes(batch.error_report_path)
    except StorageError as exc:
        raise HTTPException(status_code=503, detail='Kho lưu trữ file lỗi chưa sẵn sàng.') from exc
    return StreamingResponse(
        stream,
        media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        headers={'Content-Disposition': f'attachment; filename="udemy-progress-errors-{batch.id[:8]}.xlsx"'},
    )
...
def download_udemy_progress_export_job(
    ...
    legacy_path = (root / 'udemy-progress-exports' / filename).resolve()
    try:
        legacy_path.relative_to(root / 'udemy-progress-exports')
    except ValueError as exc:
        raise HTTPException(status_code=400, detail='Đường dẫn file export không hợp lệ.') from exc
    reference = str(result.get('storage_ref') or legacy_path)
    storage = get_object_storage()
    try:
        if not storage.exists(reference):
            raise HTTPException(status_code=410, detail='File export đã hết thời gian lưu. Hãy tạo lại báo cáo.')
        stream = storage.iter_bytes(reference)
    except StorageError as exc:
        raise HTTPException(status_code=503, detail='Kho lưu trữ file export chưa sẵn sàng.') from exc
    return StreamingResponse(
        stream,
        media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        headers={'Content-Disposition': f'attachment; filename="{filename}"'},
...
def deactivate_class_course_mapping(
    class_id: str,
    cleanup_previous_course: bool = Query(False),
    user: UserContext = Depends(require_permission('manage_settings')),
    db: Session = Depends(get_db),
):
    _require_academic_admin(db, user)
    item = AcademicService(db).deactivate_class_course_mapping(user, class_id, cleanup_previous_course=cleanup_previous_course)
...
@router.post('/campuses/seed-from-env', response_model=list[AcademicCampusOut], deprecated=True)
def seed_academic_campuses_from_env(
    ...
    raise HTTPException(
        status_code=410,
        detail={
            'code': 'ACADEMIC_CAMPUS_MANUAL_ONLY',
            'message': 'Danh mục cơ sở chỉ được nhập và chỉnh sửa thủ công tại trang /premises.',
        },
    )


@router.post('/campuses/sync-from-ap', response_model=list[AcademicCampusOut], deprecated=True)
