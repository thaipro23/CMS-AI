...
def _database_schema_state() -> dict:
    """Return a small, non-secret schema readiness snapshot.

    Production traffic must not be admitted merely because ``SELECT 1`` works.
    A backend image deployed before its Alembic Job can otherwise become Ready
    while ORM models reference columns that do not exist yet, causing endpoint
    500s. Keep this check intentionally bounded to the high-risk Question Bank
    tables used by the main teacher workflows.
    """
    inspector = inspect(engine)
    missing_tables: list[str] = []
    missing_columns: dict[str, list[str]] = {}
    for table_name, required_columns in _CORE_SCHEMA_REQUIREMENTS.items():
        if not inspector.has_table(table_name):
            missing_tables.append(table_name)
            continue
        actual = {str(column.get('name')) for column in inspector.get_columns(table_name)}
    ...
    if inspector.has_table('alembic_version'):
        try:
            with engine.connect() as conn:
                alembic_versions = [str(row[0]) for row in conn.execute(text('SELECT version_num FROM alembic_version')).all()]
        except Exception:
            alembic_versions = []

    migration_ready = _EXPECTED_ALEMBIC_REVISION in alembic_versions
    return {
        'ready': not missing_tables and not missing_columns and migration_ready,
        'expected_alembic_revision': _EXPECTED_ALEMBIC_REVISION,
        'migration_ready': migration_ready,
        'alembic_versions': sorted(alembic_versions),
...
@router.get('/health')
def health():
    # Liveness only: process is running. Kubernetes readiness uses /health/ready.
    return {'status': 'ok', 'version': settings.app_version}


@router.get('/health/ready')
def ready_health():
    """Kubernetes readiness: DB reachable and critical schema migrated."""
    try:
        with engine.connect() as conn:
            conn.execute(text('SELECT 1'))
        schema = _database_schema_state()
        if not schema['ready']:
            raise HTTPException(
                ...
                detail={
                    ...
                    'message': 'Database chưa chạy đủ migration cho backend image hiện tại.',
                },
            )
        return {
            'status': 'ok',
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail={'status': 'not_ready', 'database': 'unreachable', 'error_type': exc.__class__.__name__},
        ) from exc
...
def db_health(user: UserContext = Depends(require_permission('manage_settings'))):
    ...
    try:
        ...
        schema = _database_schema_state()
        return {
            'status': 'ok' if schema['ready'] else 'degraded',
            'database': 'reachable',
            'schema': schema,
...
def storage_health(user: UserContext = Depends(require_permission('manage_settings'))):
    """Read-only external MinIO readiness check without exposing secrets."""
    del user
    try:
        return get_object_storage().health(write_test=False)
    except StorageError as exc:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail={'status': 'error', 'storage': 'unreachable', 'message': str(exc)},
        ) from exc


@router.post('/health/storage/smoke')
def storage_write_smoke(user: UserContext = Depends(require_permission('manage_settings'))):
    """Explicit authenticated PUT/GET/DELETE probe for deployment verification."""
    del user
    try:
        return get_object_storage().health(write_test=True)
    except StorageError as exc:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail={'status': 'error', 'storage': 'write_test_failed', 'message': str(exc)},
        ) from exc
