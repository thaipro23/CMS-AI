...
class OpenEdXConnectorClient:
    ...
    def __init__(self) -> None:
        ...
        self.enrollment_remove_endpoint = _path(
            getattr(settings, 'openedx_connector_enrollment_remove_endpoint', None),
            '/api/ai-connector/v1/course-enrollment/remove',
        )
    ...
    def _long_request_timeout_seconds(self) -> int:
        """Connector calls that may touch many LMS rows but still traverse HAProxy."""
        return max(int(self.timeout_seconds or 0), 50)
    ...
    def verify_course_exists(self, course_id: str) -> dict[str, Any]:
        """Strict tri-state verification for a physical Open edX Course."""
        target = str(course_id or '').strip()
        if not target:
            return {'available': True, 'exists': False, 'error_code': 'OPENEDX_COURSE_ID_EMPTY', 'source': 'input'}
        if self.configured():
            try:
                rows = self._search_courses_via_connector(query=target, exact_course_id=target, limit=10)
                exact = [item for item in rows if str(item.get('course_id') or '').strip().lower() == target.lower()]
                if len(exact) == 1:
                    return {'available': True, 'exists': True, 'course_id': str(exact[0]['course_id']), 'display_name': exact[0].get('display_name'), 'source': 'openedx_connector_course_search'}
                if len(exact) > 1:
                    return {'available': True, 'exists': False, 'error_code': 'OPENEDX_COURSE_AMBIGUOUS', 'match_count': len(exact), 'source': 'openedx_connector_course_search'}
                return {'available': True, 'exists': False, 'error_code': 'OPENEDX_COURSE_NOT_FOUND', 'match_count': 0, 'source': 'openedx_connector_course_search'}
            except Exception:
                pass
        base = (settings.openedx_lms_base_url or settings.openedx_base_url or '').rstrip('/')
        if base:
            try:
                item = self._get_course_via_lms_courses_api_exact(course_id=target)
                if item and str(item.get('course_id') or '').strip().lower() == target.lower():
                    return {'available': True, 'exists': True, 'course_id': str(item['course_id']), 'display_name': item.get('display_name'), 'source': 'lms_courses_api_exact'}
                return {'available': True, 'exists': False, 'error_code': 'OPENEDX_COURSE_NOT_FOUND', 'match_count': 0, 'source': 'lms_courses_api_exact'}
            except Exception:
                pass
        return {'available': False, 'exists': False, 'error_code': 'OPENEDX_COURSE_VALIDATION_UNAVAILABLE', 'source': 'unavailable'}

    def remove_course_access(self, *, course_id: str, students: list[dict[str, Any]] | None = None, teachers: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
        if not self.configured():
            raise RuntimeError('Chưa cấu hình Open edX Connector/HMAC để dọn Course cũ')
        data = self._post_json(
            ...
            timeout=self._long_request_timeout_seconds(),
            legacy_fallback=False,
        )
        if isinstance(data, dict):
            rows = data.get('results') or []
            if data.get('ok') is False:
                raise RuntimeError(str(data.get('message') or 'Open edX không xác nhận cleanup Course cũ thành công'))
