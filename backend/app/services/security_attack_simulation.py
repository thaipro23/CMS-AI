...
class SecurityAttackSimulationService:
    ...
    def _cases(self) -> list[AttackCase]:
        ...
        return [
            ...
            self._case(3, 'auth', 'Wrong/expired token or CMS bridge replay', 'JWT decode enforces issuer/audience/token type and checks revoked jti; CMS bridge ticket is claimed once in Redis.', self._has(security, 'issuer=settings.jwt_issuer', 'audience=settings.jwt_audience', "token_type') != 'ai_session", 'is_session_revoked') and self._has('backend/app/core/session_security.py', 'claim_bridge_ticket_once', 'nx=True', 'session-revoked'), f'{security}; backend/app/core/session_security.py', 'Keep issuer/audience/token_type checks, one-time jti claim and Redis revocation enabled.'),
