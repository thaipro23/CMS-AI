...
class LearningAnalyticsResultsWorkflowService:
    ...
    def class_behavior_overview(
        ...
        *,
        ...
        effective_by_class = AcademicService(self.db).effective_course_mappings_for_classes(classes)
        ...
        for klass in classes:
            ...
            mapping = effective_by_class.get(class_id)
            openedx_course_id = str(getattr(mapping, 'openedx_course_id', '') or '').strip() if mapping else ''
            parsed_course = _parse_openedx_course_id(openedx_course_id) if openedx_course_id else None
            is_class_override = bool(mapping and str(getattr(mapping, 'class_id', '') or '') == class_id)
            item = {
                'class_id': class_id,
                ...
                'campus': klass.campus,
                'branch': klass.branch,
                'openedx_course_id': openedx_course_id or None,
                'openedx_org': parsed_course.get('org') if parsed_course else None,
                'openedx_mapping_source': 'class_override' if is_class_override else ('subject_term_mapping' if mapping else None),
