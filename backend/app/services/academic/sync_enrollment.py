...
def _assert_live_course_for_mutation(self, course_id: str) -> dict[str, Any]:
        ...
        if not live.get('available'):
            raise HTTPException(status_code=503, detail='Không xác minh được Course CMS/Open edX lúc này. Chưa tạo user/enroll để tránh ghi nhầm Course.')
        if not live.get('exists'):
            raise HTTPException(status_code=400, detail=f'Course CMS/Open edX không tồn tại: {course_id}. Hãy sửa mapping trước khi đồng bộ.')
        return live
...
def sync_class_course_enrollment(self, user: UserContext, class_id: str, *, force: bool = False, limit: int = 1000, mode: str | None = None, ensure_users: bool = True) -> dict[str, Any]:
        ...
        limit = self._normalize_class_sync_limit(limit)
        self._assert_live_course_for_mutation(course_id)
        ...
        if teacher_payload:
            for start in range(0, len(teacher_payload), batch_size):
                ...
                for teacher, payload in chunk:
                    ...
                    by_course = dict(existing.get('course_staff_by_course') or {}) if isinstance(existing.get('course_staff_by_course'), dict) else {}
                    by_course[course_id] = staff_record
                    teacher.metadata_json = {
                        **_json_safe_value(existing),
                        'course_staff_by_course': _json_safe_value(by_course),
                        'course_staff': _json_safe_value(staff_record),
                    }
