...
class UdemyProgressService:
    ...
    MAX_ARCHIVE_ENTRIES = max(200, MAX_FILES * 4)
    REPORT_TERM_RE = re.compile(r'(?i)(?:^|[^A-Z0-9])(SP|SU|FA)\s*[-_ ]?\s*(20\d{2})(?:\b|_)')
    REPORT_SUBJECT_RE = re.compile(r'(?i)\b(?:poly|ptcd)\s*-\s*([A-Z][A-Z0-9]{2,31})\s*-')
    ...
    def validate_upload_metadata(cls, *, filename: str, content_type: str | None) -> None:
        ...
        if suffix not in cls.ALLOWED_EXTENSIONS | cls.ARCHIVE_EXTENSIONS:
            raise ValueError('Chỉ chấp nhận file tiến độ Udemy .xlsx, .csv hoặc gói .zip.')
        clean_type = str(content_type or '').split(';', 1)[0].strip().lower()
        if clean_type not in cls.ALLOWED_CONTENT_TYPES | {'application/x-zip-compressed'}:
            raise ValueError('Content-Type của file không phù hợp với .xlsx/.csv/.zip.')
        if suffix == '.zip' and clean_type not in {'', 'application/octet-stream', 'application/zip', 'application/x-zip-compressed'}:
            raise ValueError('Content-Type của gói ZIP không hợp lệ.')

    @classmethod
    def _safe_archive_member_name(cls, value: str) -> str:
        normalized = str(value or '').replace('\\', '/')
        parts = [part for part in normalized.split('/') if part not in {'', '.'}]
        if normalized.startswith('/') or '..' in parts:
            raise ValueError('Gói ZIP chứa đường dẫn nội bộ không an toàn.')
        return Path(normalized).name

    @classmethod
    def expand_upload_payload(cls, *, filename: str, content_type: str | None, raw: bytes) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        """Return logical CSV/XLSX files from a direct upload or a ZIP archive.

        Archive extraction stays in memory, rejects path traversal/encryption/zip-bomb
        patterns and never recursively expands nested archives. Unsupported archive
        members are reported to the caller instead of aborting valid siblings.
        """
        cls.validate_upload_metadata(filename=filename, content_type=content_type)
        suffix = Path(str(filename or '')).suffix.lower()
        if suffix != '.zip':
            return [{'filename': Path(filename).name, 'raw': raw, 'source_archive': None}], []
        if not raw:
            raise ValueError('Gói ZIP Udemy rỗng.')
        if len(raw) > cls.MAX_TOTAL_UPLOAD_BYTES:
            limit_mb = max(1, cls.MAX_TOTAL_UPLOAD_BYTES // (1024 * 1024))
            raise ValueError(f'Gói ZIP vượt giới hạn upload {limit_mb} MB.')
        ...
        try:
            with zipfile.ZipFile(BytesIO(raw)) as archive:
                ...
                if len(entries) > cls.MAX_ARCHIVE_ENTRIES:
                    raise ValueError('Gói ZIP có quá nhiều thành phần để xử lý an toàn.')
                for info in entries:
                    if info.is_dir():
                        continue
                    member_name = cls._safe_archive_member_name(info.filename)
                    if not member_name or info.filename.replace('\\', '/').startswith('__MACOSX/'):
                        continue
                    if info.flag_bits & 0x1:
                        raise ValueError('Gói ZIP chứa file được mã hóa bằng mật khẩu.')
                    file_size = max(0, int(info.file_size or 0))
                    compressed_size = max(0, int(info.compress_size or 0))
                    total_uncompressed += file_size
                    if total_uncompressed > cls.MAX_TOTAL_UPLOAD_BYTES:
                        limit_mb = max(1, cls.MAX_TOTAL_UPLOAD_BYTES // (1024 * 1024))
                        raise ValueError(f'Tổng dữ liệu giải nén vượt giới hạn {limit_mb} MB.')
                    if file_size > cls.MAX_FILE_BYTES:
                        rejected.append({
                            'file_name': member_name, 'source_archive': Path(filename).name,
                            'reason_code': 'FILE_TOO_LARGE',
                            'message': f'File vượt giới hạn {max(1, cls.MAX_FILE_BYTES // (1024 * 1024))} MB.',
                        })
                        continue
                    if compressed_size and file_size > max(1024 * 1024, compressed_size * cls.MAX_XLSX_COMPRESSION_RATIO):
                        raise ValueError('Gói ZIP có tỷ lệ nén bất thường, có nguy cơ zip bomb.')
                    member_suffix = Path(member_name).suffix.lower()
                    if member_suffix not in cls.ALLOWED_EXTENSIONS:
                        if member_name.startswith('.'):
                            continue
                        rejected.append({
                            'file_name': member_name, 'source_archive': Path(filename).name,
                            'reason_code': 'UNSUPPORTED_FILE',
                            'message': 'Chỉ xử lý file .csv hoặc .xlsx bên trong ZIP.',
                        })
                        continue
                    try:
                        ...
                        rejected.append({
                            'file_name': member_name, 'source_archive': Path(filename).name,
                            'reason_code': 'ARCHIVE_READ_FAILED',
                            'message': 'Không đọc được file bên trong ZIP.',
                        })
                        continue
                    members.append({'filename': member_name, 'raw': member_raw, 'source_archive': Path(filename).name})
        except zipfile.BadZipFile as exc:
            raise ValueError('Không đọc được gói ZIP. Hãy tạo lại file .zip hợp lệ.') from exc
        if not members and not rejected:
            raise ValueError('Gói ZIP không chứa file Udemy .csv hoặc .xlsx.')
        if len(members) > cls.MAX_FILES:
            raise ValueError(f'Mỗi lần tối đa {cls.MAX_FILES} file Udemy sau khi giải nén ZIP.')
    ...
    @classmethod
    def _report_title_from_rows(cls, rows: list[list[Any]]) -> str | None:
        title_column = None
        header_index = None
        aliases = {'tieu de lo trinh', 'learning path title', 'path title'}
        for row_index, row in enumerate(rows[:15]):
            for column_index, value in enumerate(row):
                if cls.normalize_text(value) in aliases:
                    header_index = row_index
                    title_column = column_index
                    break
            if title_column is not None:
                break
        if title_column is None or header_index is None:
            return None
        for row in rows[header_index + 1:]:
            if title_column < len(row) and str(row[title_column] or '').strip():
                return str(row[title_column]).strip()
        return None

    @classmethod
    def inspect_report_identity(cls, *, filename: str, raw: bytes) -> dict[str, str | None]:
        """Read lightweight subject/term/branch hints from the Udemy path title."""
        suffix = Path(str(filename or '')).suffix.lower()
        rows: list[list[Any]] = []
        if suffix == '.csv':
            ...
            try:
                ...
                return {'path_title': None, 'subject_code': None, 'term_code': None, 'branch': None}
            try:
                if wb.worksheets:
                    for row in wb.worksheets[0].iter_rows(min_row=1, max_row=40, values_only=True):
                        rows.append(list(row))
            finally:
                wb.close()
        title = cls._report_title_from_rows(rows)
        if not title:
            return {'path_title': None, 'subject_code': None, 'term_code': None, 'branch': None}
        subject_match = cls.REPORT_SUBJECT_RE.search(title)
        term_match = cls.REPORT_TERM_RE.search(title)
        normalized_title = cls.normalize_text(title)
        branch = 'ptcd' if normalized_title.startswith('ptcd ') or normalized_title == 'ptcd' else ('poly' if normalized_title.startswith('poly ') or normalized_title == 'poly' else None)
        return {
            'path_title': title,
            'subject_code': subject_match.group(1).upper() if subject_match else None,
            'term_code': f'{term_match.group(1).upper()}{term_match.group(2)}' if term_match else None,
            'branch': branch,
        }

    @classmethod
    def subject_code_from_filename(cls, filename: str) -> str | None:
        stem = Path(str(filename or '')).stem.strip()
        match = re.match(r'^([A-Za-z]{2,}[A-Za-z0-9]*)[_\-\s]', stem)
        code = (match.group(1) if match else stem.split('_', 1)[0]).strip().upper()
        return code if re.fullmatch(r'[A-Z][A-Z0-9]{2,31}', code) else None

    @classmethod
    def expected_term_tokens(cls, term: AcademicTerm) -> set[str]:
        tokens: set[str] = set()
        code = re.sub(r'[^A-Z0-9]', '', str(term.term_code or '').upper())
        if code:
            tokens.add(code)
            short = re.fullmatch(r'(SP|SU|FA)(\d{2})', code)
            if short:
                tokens.add(f'{short.group(1)}20{short.group(2)}')
        normalized_name = cls.normalize_text(term.term_name)
        year_match = re.search(r'\b(20\d{2})\b', str(term.term_name or ''))
        if year_match:
            season = None
            if 'summer' in normalized_name or 'he ' in f'{normalized_name} ': season = 'SU'
            elif 'spring' in normalized_name or 'xuan' in normalized_name: season = 'SP'
            elif 'fall' in normalized_name or 'autumn' in normalized_name or 'thu ' in f'{normalized_name} ': season = 'FA'
            if season:
                tokens.add(f'{season}{year_match.group(1)}')
        return tokens
    ...
    def cleanup_expired_artifacts(
        ...
        *,
        ...
        if root is None and str(settings.storage_provider or 'local').strip().lower() == 'minio':
            storage = get_object_storage()
            for prefix, cutoff, artifact_kind in [
                ('udemy-progress-imports', import_cutoff, 'import'),
                ('udemy-progress-exports', export_cutoff, 'export'),
            ]:
                for item in storage.list_objects(prefix):
                    ...
                    if artifact_kind == 'import' and relative_parts and relative_parts[0] in protected_import_ids:
                        continue
                    if artifact_kind == 'export' and relative_parts and relative_parts[-1] in protected_export_names:
                        continue
                    ...
                    if modified.timestamp() < cutoff:
                        ...
                        deleted_files += 1
            return {'deleted_files': deleted_files, 'deleted_dirs': 0, 'scanned': scanned}
    ...
    def resolve_delivery(
        ...
        *,
        ...
        if delivery_id:
            ...
            subject_code = str(subject_code_hint or self.subject_code_from_filename(filename) or '').strip().upper()
            if not re.fullmatch(r'[A-Z][A-Z0-9]{2,31}', subject_code):
                raise ValueError(f'Không xác định được mã môn từ tên file “{filename}”. Dùng dạng SOF3032_*.xlsx/.csv hoặc báo cáo có Tiêu đề lộ trình hợp lệ.')
    ...
    def process_batch(self, batch: UdemyProgressImportBatch) -> dict[str, Any]:
        ...
        storage = get_object_storage()
        try:
            with storage.local_copy(str(batch.file_path or ''), suffix=Path(batch.file_name or '').suffix) as path:
                now = datetime.utcnow()
                plan, milestone = self._active_plan_context(delivery.id, now.date())
                parsed = self.parse_path(path, active_plan=plan)
        except StorageError as exc:
            raise ValueError('File import không còn tồn tại trong vùng lưu trữ dùng chung.') from exc
        ...
        batch.error_report_path = report_path or None
        self.db.add(batch)
        self.db.commit()
        self.db.refresh(batch)
        return dict(batch.result_json or {})

    def _write_error_report(self, batch: UdemyProgressImportBatch, subject: AcademicSubject) -> str | None:
        ...
        if not rows:
            return None
        filename = f'udemy-progress-errors-{subject.subject_code}-{batch.id[:8]}.xlsx'
        wb = Workbook()
        ws = wb.active
        ws.title = 'Lỗi import'
        ...
        wb.save(raw)
        wb.close()
        return get_object_storage().put_bytes(
            f'udemy-progress-imports/errors/{filename}',
            raw.getvalue(),
