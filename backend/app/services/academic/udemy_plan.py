...
class UdemyPlanService:
    ...
    def _preview_dir() -> Path:
        path = Path(settings.local_storage_path) / 'udemy-plan-previews'
    ...
    def _cleanup_expired_previews(cls) -> None:
        ...
        for path in cls._preview_dir().glob('*.json'):
            try:
    ...
    def persist_preview(self, preview: dict[str, Any]) -> str:
        ...
        path = self._preview_dir() / f'{token}.json'
        temporary = path.with_suffix('.tmp')
        stored_preview = dict(preview)
        stored_preview['preview_token'] = token
        temporary.write_text(json.dumps(json_safe_value(stored_preview), ensure_ascii=False, separators=(',', ':')), encoding='utf-8')
        temporary.replace(path)
        return token

    def load_preview(self, token: str, *, requested_by: str | None) -> tuple[Path, dict[str, Any]]:
        ...
        if not path.exists() or not path.is_file():
            raise HTTPException(status_code=404, detail='Preview đã hết hạn hoặc không tồn tại. Hãy upload lại file.')
        modified = datetime.fromtimestamp(path.stat().st_mtime, tz=timezone.utc)
        if datetime.now(timezone.utc) - modified > timedelta(hours=self.PREVIEW_TTL_HOURS):
            path.unlink(missing_ok=True)
