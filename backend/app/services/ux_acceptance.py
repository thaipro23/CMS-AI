...
class UxAcceptanceService:
    ...
    def _checks(self) -> list[UxAcceptanceCheck]:
        ...
        checks.extend([
            UxAcceptanceCheck('accessibility', 'TABLE_HORIZONTAL_SCROLL_ACCESSIBLE', 'BLOCKER', 'tabIndex={0}' in enterprise and 'data-column-contract="full-content"' in enterprise and 'chỉ cuộn ngang' in enterprise, 'Table container is keyboard-focusable, labeled and keeps the full-content contract.', 'Giữ toàn bộ nội dung cột, cho xuống dòng và chỉ cuộn trong container khi thực sự cần.', 'frontend/components/table/EnterpriseDataTable.tsx'),
