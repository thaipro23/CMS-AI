...
class VersionedQuestionBankService:
    ...
    BANK_REJECTED_STATUSES = {'rejected'}
    BANK_KNOWN_STATUSES = BANK_PENDING_STATUSES | BANK_ERROR_STATUSES | BANK_APPROVED_STATUSES | BANK_REJECTED_STATUSES
    ...
    def _bank_question_status_counts(self, questions: list[Question]) -> dict:
        ...
        rejected = [q for q in active if q.status in self.BANK_REJECTED_STATUSES]
        # Legacy/unknown statuses must block review completion. Older logic simply
        # ignored them, which could make a chapter look release-ready while dirty
        # rows were still present.
        unknown = [q for q in active if q.status not in self.BANK_KNOWN_STATUSES]
        unresolved = pending + draft_error + unknown
        total = len(active)
        return {
            ...
            'status': 'ready' if bool(approved) and not unresolved else ('needs_fix' if (draft_error or unknown) else ('needs_review' if pending else ('empty' if not active else 'not_ready'))),
        }
        ...
        for src_bv in self.db.query(QuestionBankVersion).filter(QuestionBankVersion.subject_offering_id == source.id).order_by(QuestionBankVersion.created_at.asc()).all():
            ...
            for src_q in questions:
                ...
                new_q = Question(
                    ...
                    pedagogy_json=dict(src_q.pedagogy_json or {}),
    ...
    def _safe_delete_material_file(self, material: LearningMaterialVersion) -> dict[str, Any]:
        """Delete a legacy local file or private MinIO object when it is safe.

        Legacy paths remain constrained to ``LOCAL_STORAGE_PATH`` by the storage
        adapter. Cloned versions can share one reference, so the physical object
        is retained until the last database reference disappears.
        """
        ...
        try:
            ...
            stored = storage.stat(raw_path)
            if not stored:
                return {'deleted': False, 'skipped': False, 'reason': 'file_not_found', 'path': raw_path}
            storage.delete(raw_path, missing_ok=True)
            return {'deleted': True, 'skipped': False, 'bytes_deleted': int(stored.size), 'path': raw_path}
        except (StorageError, OSError) as exc:
            return {'deleted': False, 'skipped': True, 'reason': 'delete_error', 'error_type': exc.__class__.__name__, 'path': raw_path}
    ...
    def carry_over_questions(self, *, from_bank_version_id: str, to_bank_version_id: str, question_ids: list[str] | None = None, require_review: bool = False, actor: str | None = None, diff_id: str | None = None) -> dict:
        ...
        for source_question in candidates:
            ...
            q = Question(
                ...
                learning_objective=source_question.learning_objective,
                pedagogy_json=dict(source_question.pedagogy_json or {}),
    ...
    def upload_material_bytes(
        ...
        *,
        ...
        storage = get_object_storage()
        storage_key = f'question-bank/{version.id}/{material_id}-{filename}'
        storage_path = storage.reference_for_key(storage_key)
        ...
        try:
            stored_reference = storage.put_bytes(storage_key, raw, content_type=content_type or None)
            material.storage_path = stored_reference
            self.db.commit()
        except Exception:
            self.db.rollback()
            if stored_reference:
                try:
                    storage.delete(stored_reference, missing_ok=True)
                except Exception:
                    pass
            raise
    ...
    def bank_material_cleanup_health(self) -> dict[str, Any]:
        ...
        for material in deleted_rows[:2000]:
            ...
            if not path:
                continue
            try:
                stored = storage.stat(path)
                if stored:
                    deleted_files += 1
                    deleted_file_bytes += int(stored.size)
        ...
        return {
            ...
            'deleted_tombstones_with_storage_object': int(deleted_files),
            'deleted_storage_bytes_estimate': int(deleted_file_bytes),
            # Backward-compatible names retained for the existing dashboard.
    ...
    def create_manual_bank_question(self, *args, **kwargs):
        return self._generation_review_workflow().create_manual_bank_question(*args, **kwargs)
    ...
    def _cleanup_stale_release_keys_for_chapter(self, *, chapter: SubjectChapter, reason: str) -> int:
        return self._release_publish_workflow()._cleanup_stale_release_keys_for_chapter(chapter=chapter, reason=reason)
    ...
    def cancel_failed_release(self, *, release_id: str, actor: str | None = None) -> dict:
        return self._release_publish_workflow().cancel_failed_release(release_id=release_id, actor=actor)
    ...
    def _format_offering_candidate(self, item: dict) -> dict:
        return self._quiz_creation_workflow()._format_offering_candidate(item)

    def _select_offering_for_course(self, *, course_id: str, subject: Subject, selected_subject_offering_id: str | None = None) -> tuple[SubjectOffering | None, list[dict], list[str]]:
        return self._quiz_creation_workflow()._select_offering_for_course(course_id=course_id, subject=subject, selected_subject_offering_id=selected_subject_offering_id)
    ...
    def create_quiz_blueprint(self, *, subject_id: str, chapter_id: str, subject_offering_id: str | None = None, title: str, total_questions: int, difficulty_easy: int, difficulty_medium: int, difficulty_hard: int, max_families_per_bank: int = 2, pick_count_per_slot: int = 1, single_select_count: int | None = None, multi_select_count: int | None = None, text_input_count: int | None = None, numerical_input_count: int | None = None) -> QuizBlueprint:
        ...
        if not subject or not chapter: raise ValueError('Không tìm thấy Subject hoặc Chapter cho Quiz Blueprint.')
        if str(chapter.subject_id)!=str(subject_id): raise ValueError('Chapter không thuộc Subject của Quiz Blueprint.')
        if subject_offering_id:
            offering=self.db.get(SubjectOffering,subject_offering_id)
    ...
    async def publish_release_to_openedx(self, *, release_id: str, actor: str | None = None, course_id_for_org: str | None = None, force_reimport: bool = False, progress_callback: Any | None = None) -> dict:
        return await self._release_publish_workflow().publish_release_to_openedx(release_id=release_id, actor=actor, course_id_for_org=course_id_for_org, force_reimport=force_reimport, progress_callback=progress_callback)
