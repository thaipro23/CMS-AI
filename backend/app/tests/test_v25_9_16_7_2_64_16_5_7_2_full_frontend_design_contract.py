...
from app.schemas.rbac import RoleAssignmentBatchCreate, RoleAssignmentCreate
...
def test_bank_dashboard_and_chapter_use_compact_action_contracts():
    ...
    assert '<BankPageIdentity' in dashboard
    assert chapter.count('chapter-inline-stats') <= 1
    assert 'Tạo câu hỏi (${usedQuestionCount}/${chapterQuestionLimit})' in chapter
    assert 'Duyệt câu hỏi (${pendingReviewCount} câu chờ duyệt)' in chapter
...
def test_student_pages_use_contextual_actions_and_standard_table():
    subjects = source('frontend/app/student-management/StudentManagementPlatformPage.tsx')
    detail = source('frontend/app/student-management/classes/[classId]/page.tsx')
    assert 'title={`Danh sách môn ${platformLabel}`}' in subjects
...
def test_legacy_campus_manager_cannot_be_granted_in_single_assignment_or_import_template():
    with pytest.raises(ValidationError):
    ...
    assert 'CAMPUS_OWNER,CAMPUS_MANAGER' not in route
    assert 'CAMPUS_OWNER,TEACHER_ASSIGNED' in route
...
def test_rbac_ui_and_backend_support_atomic_multi_scope_grant():
    ...
    assert "!['CAMPUS_MANAGER', 'TEACHER_ASSIGNED'].includes(role.code)" in users
    schema = source('backend/app/schemas/rbac.py')
    assert "value == 'CAMPUS_MANAGER'" in schema
    assert "role_code == CAMPUS_MANAGER" in service
