...
def test_version_is_synchronized_across_backend_frontend_and_footer():
    ...
    compose = read('docker-compose.prod.yml')
    assert "app_version: str = '25.9.16.7.2.37'" in config
    assert '"version": "25.9.16.7.2.37"' in package
    assert 'NEXT_PUBLIC_APP_VERSION' in shell
    assert 'NEXT_PUBLIC_APP_VERSION: ${APP_VERSION:-25.9.16.7.2.37}' in compose
