...
class Settings(BaseSettings):
    ...
    # Enrollment writes hit primary while class analytics reads the configured
    # replica. Queue grade/progress as a second pass after this bounded delay.
    academic_learning_sync_replica_delay_seconds: int = 60
...
def validate_security_settings() -> None:
    ...
    if settings.academic_learning_sync_replica_delay_seconds < 10 or settings.academic_learning_sync_replica_delay_seconds > 900:
        errors.append('ACADEMIC_LEARNING_SYNC_REPLICA_DELAY_SECONDS must be between 10 and 900')
