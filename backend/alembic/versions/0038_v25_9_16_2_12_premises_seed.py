...
def upgrade() -> None:
    conn = op.get_bind()
    op.execute(sa.text(
        'ALTER TABLE academic_campuses ALTER COLUMN created_at SET DEFAULT CURRENT_TIMESTAMP'
    ))
    op.execute(sa.text(
        'ALTER TABLE academic_campuses ALTER COLUMN updated_at SET DEFAULT CURRENT_TIMESTAMP'
    ))
    for item in PREMISES:
        ...
        if existing:
            ...
            conn.execute(
                sa.text(
                    'INSERT INTO academic_campuses ('
                    'id, campus_code, campus_name, branch, active, sort_order, '
