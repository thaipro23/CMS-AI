...
def upgrade() -> None:
    ...
    op.create_index('ix_ai_question_search_bank_version', 'ai_question_search_documents', ['bank_version_id'])
    op.create_index('ix_ai_question_search_subject', 'ai_question_search_documents', ['subject_id'])
    op.create_index('ix_ai_question_search_offering', 'ai_question_search_documents', ['subject_offering_id'])
    op.create_index('ix_ai_question_search_chapter', 'ai_question_search_documents', ['chapter_id'])
    op.create_index('ix_ai_question_search_status', 'ai_question_search_documents', ['status'])
    op.create_index('ix_ai_question_search_difficulty', 'ai_question_search_documents', ['difficulty'])
    op.create_index('ix_ai_question_search_family', 'ai_question_search_documents', ['question_family_id'])
    op.create_index('ix_ai_question_search_bank_status', 'ai_question_search_documents', ['bank_version_id', 'status'])
    op.create_index('ix_ai_question_search_subject_chapter_status', 'ai_question_search_documents', ['subject_id', 'chapter_id', 'status'])
    op.create_index('ix_ai_question_search_chapter_difficulty', 'ai_question_search_documents', ['chapter_id', 'difficulty'])
    op.create_index('ix_ai_question_search_updated', 'ai_question_search_documents', ['updated_at'])

    if _is_postgresql():
        # pg_trgm speeds ILIKE/LIKE '%token%' queries. unaccent is installed for
        # future DB-side normalization; current question documents are already
        # accent-stripped by application code so the GIN index remains valid.
        op.execute('CREATE EXTENSION IF NOT EXISTS pg_trgm')
        op.execute('CREATE EXTENSION IF NOT EXISTS unaccent')
        # Generated tsvector keeps the planned search_vector available for later
        # ranking/full-text work without asking the API path to maintain it.
        op.add_column(
            'ai_question_search_documents',
            sa.Column(
                'search_vector',
