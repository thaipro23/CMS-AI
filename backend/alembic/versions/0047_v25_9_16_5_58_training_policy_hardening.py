...
def _has_unique(table: str, name: str) -> bool:
    ...
    names.update(item.get('name') for item in inspector.get_indexes(table) if item.get('unique'))
...
def upgrade() -> None:
    ...
    if _has_unique('academic_quiz_deadline_overrides', 'uq_academic_quiz_deadline_class_course_number'):
        op.drop_constraint('uq_academic_quiz_deadline_class_course_number', 'academic_quiz_deadline_overrides', type_='unique')
    if _has_unique('academic_assignment_defense_scores', 'uq_academic_assignment_defense_class_student_key'):
