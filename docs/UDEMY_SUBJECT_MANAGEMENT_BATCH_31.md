...
Worker gọi nguồn AP `GET /get-course?branch=poly&term_name=<kỳ>`, upsert danh mục môn và tạo các bản ghi triển khai theo Học kỳ/Block. F5 hoặc rời trang không làm mất tác vụ; trạng thái được lưu trong `academic_bulk_operation_jobs` và hiển thị ở `/jobs`.
