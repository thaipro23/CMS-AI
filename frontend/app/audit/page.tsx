...
function errorText(v?: string | null) {
  return v === 'USER_ERROR' ? 'Do người dùng/cấu hình' : v === 'SYSTEM_ERROR' ? 'Do hệ thống' : v === 'EXTERNAL_SERVICE_ERROR' ? 'Do dịch vụ ngoài' : v === 'VALIDATION_ERROR' ? 'Lỗi dữ liệu đầu vào' : v === 'AUTH_ERROR' ? 'Lỗi phân quyền' : '—'
}
...
export default function AuditPage() {
  ...
  return <div className="page-stack ops-console audit-console">
    <section className="ops-hero card">
      <div><span className="eyebrow">Audit scope</span><h1>Nhật ký hoạt động</h1><p>Theo dõi thao tác, lỗi và thay đổi trong phạm vi quyền của bạn. Dữ liệu nhạy cảm đã được lọc từ backend.</p></div>
    ...
    <section className="card"><PaginationControls page={page} pageSize={pageSize} total={total} totalPages={totalPages} onPageChange={setPage} onPageSizeChange={(s) => { setPageSize(s); setPage(1) }} loading={loading} label="log" /><div className="audit-timeline">{filteredRows.length ? filteredRows.map((row) => <article className={`audit-card ${row.status === 'failed' ? 'failed' : 'success'}`} key={row.id}><div className="audit-marker" /><div className="audit-body"><div className="audit-top"><b>{actionText(row.action)}</b><StatusBadge status={row.status} /></div><p>{row.message || 'Không có nội dung mô tả.'}</p><div className="audit-meta"><span>{formatDate(row.created_at)}</span><span>{actorLabel(row)} · {row.actor_role || '—'}</span><span>{row.target_type || 'target'} {row.target_id || ''}</span><span>{errorText(row.error_type)}</span></div></div></article>) : <div className="empty-state">Không có log phù hợp với bộ lọc.</div>}</div></section>
  </div>
}
