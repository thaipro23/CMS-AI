...
export default function AuditPage() {
  ...
  return <div className="page-stack ops-console audit-console">
    ...
    <section className="card"><PaginationControls page={page} pageSize={pageSize} total={total} totalPages={totalPages} onPageChange={setPage} onPageSizeChange={(s) => { setPageSize(s); setPage(1) }} loading={loading} label="log" />
      <div className="responsive-table-wrap"><table className="ops-data-table audit-data-table"><thead><tr><th>Thời điểm</th><th>Người thực hiện</th><th>Hành động</th><th>Kết quả</th><th>Nguồn lỗi</th><th>Đối tượng</th><th>Nội dung</th></tr></thead><tbody>{filteredRows.length ? filteredRows.map((row) => <tr key={row.id} className={`row-${row.status}`}><td><small>{formatDate(row.created_at)}</small></td><td><b>{actorLabel(row)}</b><small>{row.actor_role || '—'}</small></td><td><b>{actionText(row.action)}</b>{debugMode ? <small>{row.action}</small> : null}</td><td><StatusBadge status={row.status} /></td><td>{errorText(row.error_type)}</td><td><small>{debugMode ? targetLabel(row) : (row.target_type || '—')}</small></td><td><span className={row.status === 'failed' ? 'table-error-text' : ''}>{row.message || 'Không có nội dung mô tả.'}</span></td></tr>) : <tr><td colSpan={7}><div className="empty-state">Không có log phù hợp với bộ lọc.</div></td></tr>}</tbody></table></div>
    </section>
  </div>
}
