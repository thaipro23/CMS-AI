...
export default function TeacherClassesPage() {
  ...
  const platform: 'cms' | 'udemy' = searchParams.get('platform') === 'udemy' ? 'udemy' : 'cms'
  ...
  const columns = useMemo<EnterpriseTableColumn<AcademicTrainingClassReport>[]>(() => {
    ...
    const platformColumns: EnterpriseTableColumn<AcademicTrainingClassReport>[] = isCms ? [
      { key: 'progress-email', header: 'Mail đã gửi', kind: 'number', minWidth: 125, priority: 'important', hideable: true, render: (cls) => <><b>{cls.progress_email_sent_count || 0} mail</b><small>Mail Send xác nhận</small></> },
