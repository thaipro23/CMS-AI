...
export function TeacherManagementPlatformPage({ platform }: { platform: TrainingPlatform }) {
  ...
  const exportExcelBackground = async () => {
    ...
    try {
      ...
      setExportJob(job);
      setMessage(noticeInfo(`Đã đưa yêu cầu xuất Excel giảng viên ${platformLabel} vào hàng đợi.`));
    } catch (error) {
