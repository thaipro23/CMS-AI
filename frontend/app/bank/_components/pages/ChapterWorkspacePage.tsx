...
${chunk.content}`).join('\n\n')
  ...
  return <PageRoot className="page-stack bank-multipage bank-contract-page bank-hierarchy-detail-page chapter-workspace-page">
    ...
    <ConfirmDialog
      ...
      description={confirmation === 'bulk_reject' ? <p>Các câu được chọn sẽ chuyển sang trạng thái Đã bỏ và được ghi Audit. Câu đã publish sẽ không bị thay đổi.</p> : confirmation === 'release_publish' ? <p>Trước khi đưa lên CMS, hệ thống đồng bộ lại Release với các câu đã duyệt hiện tại và tự loại câu lỗi/trùng. Sau đó snapshot mới được publish và khóa.</p> : <p>Release lấy các câu đã duyệt hợp lệ; câu lỗi, retired và câu trùng được tự loại. Trước lần publish đầu tiên, hệ thống đồng bộ lại snapshot theo các chỉnh sửa mới nhất. Sau khi publish thì version mới bị khóa.</p>}
