...
export function BankHistoryPage() {
  ...
  return <div className="page-stack bank-multipage ops-console history-console">
    ...
    <section className="ops-hero card">
      <div><span className="eyebrow">Release & Quiz history</span><h1>Lịch sử ngân hàng đề</h1><p>Theo dõi nhanh số lượng release, quiz, rollback và lỗi trong phạm vi được giao.</p></div>
    </section>
    ...
    <section className="card history-action-panel"><div className="button-row no-margin history-action-buttons"><button className="btn secondary" type="button" onClick={() => setQuizOpen(true)}>Quiz Open edX ({quizHistory.length})</button><button className="btn secondary" type="button" onClick={() => setReleaseOpen(true)}>Release gần đây ({releases.length})</button><Link className="btn" href="/bank/quiz">Tạo Quiz Open edX</Link></div></section>

    <Modal open={quizOpen} title="Quiz Open edX" wide onClose={() => setQuizOpen(false)}>
      <div className="modal-filter-bar"><label>Tìm trong popup<input className="input" value={q} onChange={(e) => setQ(e.target.value)} placeholder="course, release, unit..." /></label><label>Trạng thái<select className="input" value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">Tất cả</option><option value="created">Đã tạo</option><option value="published">Đã publish</option><option value="rolled_back">Đã rollback</option><option value="failed">Thất bại</option></select></label><button className="btn secondary" onClick={() => load()} type="button">Làm mới</button></div>
