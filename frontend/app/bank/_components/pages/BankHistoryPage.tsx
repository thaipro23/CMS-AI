...
export function BankHistoryPage() {
  ...
  return <div className="page-stack bank-multipage ops-console history-console">
    ...
    <Modal open={quizOpen} title="Quiz trên CMS" wide onClose={() => setQuizOpen(false)}>
      ...
      <div className="responsive-table-wrap"><table className="ops-data-table"><thead><tr><th>STT</th><th>Quiz</th><th>Khóa học</th><th>Trạng thái</th><th>Bộ đề</th><th>Bài kiểm tra</th><th>Ngày tạo</th><th>Thao tác</th></tr></thead><tbody>{filteredQuiz.map((item, index) => <tr key={item.id}><td className="stt-cell">{index + 1}</td><td><b>{item.metadata_json?.quiz_title || 'Quiz trên CMS'}</b></td><td>{item.openedx_course_id}</td><td><span className={statusClass(item.status)}>{statusLabel(item.status)}</span></td><td><small>{item.bank_release_id}</small></td><td><code>{item.openedx_unit_node_id || '—'}</code></td><td><small>{dateText(item.created_at)}</small></td><td>{can('publish_questions') && item.status !== 'rolled_back' ? <button className="btn small secondary" disabled={busy} onClick={() => run(async () => { await rollbackCourseQuizInstance(headers, item.id, { mode: 'safe', note: 'Khôi phục từ trang lịch sử Quiz' }) }, 'Đã gửi yêu cầu khôi phục Quiz', load)}>Khôi phục</button> : <span className="muted">—</span>}</td></tr>)}{!filteredQuiz.length ? <tr><td colSpan={8}><div className="empty-state">Chưa có Quiz phù hợp.</div></td></tr> : null}</tbody></table></div>
    </Modal>
    <Modal open={releaseOpen} title="Bộ đề gần đây" wide onClose={() => setReleaseOpen(false)}>
      <div className="modal-filter-bar"><label>Tìm kiếm<input className="input" value={q} onChange={(e) => setQ(e.target.value)} placeholder="khóa học, bộ đề, bài kiểm tra..." /></label><label>Trạng thái<select className="input" value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">Tất cả</option><option value="created">Đã tạo</option><option value="published">Đã đưa lên CMS</option><option value="rolled_back">Đã khôi phục</option><option value="failed">Thất bại</option></select></label><button className="btn secondary" onClick={() => load()} type="button">Làm mới</button></div>
      <div className="responsive-table-wrap"><table className="ops-data-table"><thead><tr><th>STT</th><th>Bộ đề</th><th>Trạng thái</th><th>Phiên bản câu hỏi</th><th>Thư viện CMS</th><th>Số câu</th><th>Ngày tạo</th></tr></thead><tbody>{filteredReleases.map((item, index) => <tr key={item.id}><td className="stt-cell">{index + 1}</td><td><b>{releaseTitle(item)}</b><small>{item.id}</small></td><td><span className={statusClass(item.status)}>{statusLabel(item.status)}</span></td><td><small>{item.bank_version_id}</small></td><td>{item.openedx_library_key || 'Chưa có thư viện'}</td><td>{item.approved_question_count || 0}</td><td><small>{dateText(item.created_at)}</small></td></tr>)}{!filteredReleases.length ? <tr><td colSpan={7}><div className="empty-state">Chưa có release trong phạm vi này.</div></td></tr> : null}</tbody></table></div>
    </Modal>
  </div>
}
