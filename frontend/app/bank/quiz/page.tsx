    ...
    >
      {createModal ? <>
          ...
          {createModal.kind === 'one' ? <div className="quota-preview-panel">
            <button className="btn secondary" type="button" disabled={quotaPreviewBusy || busy || Boolean(creatingKey)} onClick={previewCurrentQuota}>{quotaPreviewBusy ? 'Đang kiểm tra quota...' : 'Kiểm tra quota với Release'}</button>
            {quotaPreview ? <><div className="alert success"><b>Release đáp ứng quota.</b> {quotaPreview.assigned_question_count} câu được phân vào {quotaPreview.slots.length} Problem Bank slot.</div><div className="quiz-matrix-preview"><table><thead><tr><th>Difficulty × type</th><th>Số câu</th></tr></thead><tbody>{Object.entries(quotaPreview.matrix_target_counts || {}).filter(([, value]) => Number(value) > 0).map(([key, value]) => <tr key={key}><td>{key.replace(':', ' · ')}</td><td>{value}</td></tr>)}</tbody></table></div></> : null}
          </div> : null}
