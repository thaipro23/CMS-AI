...
import { BankReleaseQuizPlan, CourseQuizInstance, QuizAutoMapResult, QuizBlueprint, QuizChapterAction, QuizChapterPlanItem } from '../../../types'
import {
  applyQuizAutoMap,
  createQuizBlueprint,
...
export default function BankQuizPage() {
  ...
  const effectiveMappings = useMemo<EffectiveQuizMapping[]>(() => {
    return (autoMap?.mappings || []).map((item) => {
      ...
      const aggregateFinalReady = action === 'final_test' && Boolean((item as any).source_release_ids?.length)
      const effectiveReady = Boolean(effectiveRequiresQuiz && item.release_id && item.openedx_section_id && (item.ready || item.can_create !== false) && (action !== 'final_test' || aggregateFinalReady))
  ...
  async function loadBlueprintsForModal() {
    if (!createModal || createModal.kind !== 'one') {
      setBlueprints([])
    ...
    try {
      const rows = await getQuizBlueprints(headers, createModal.item.chapter_id)
      setBlueprints(rows.filter((item) => item.status === 'active'))
    } catch (error) {
      setBlueprints([])
      setMessage({ tone: 'warning', text: error instanceof Error ? `Không tải được Quiz Blueprint: ${error.message}` : 'Không tải được Quiz Blueprint.' })
    } finally {
  ...
  async function saveCurrentBlueprint() {
    ...
    setBlueprintSaveBusy(true)
    try {
      const created = await createQuizBlueprint(headers, {
        subject_id: autoMap.subject.id,
        chapter_id: createModal.item.chapter_id,
        subject_offering_id: autoMap.offering?.id || selectedOfferingId || null,
        title: blueprintTitle.trim() || `${createModal.item.action === 'final_test' ? 'Final test' : 'Quiz'} · ${createModal.item.chapter_title}`,
        total_questions: config.totalQuestions,
        difficulty_easy: config.easy,
        difficulty_medium: config.medium,
        difficulty_hard: config.hard,
      ...
      setBlueprints((current) => [created, ...current.filter((item) => item.id !== created.id)])
      setSelectedBlueprintId(created.id)
      setMessage({ tone: 'success', text: `Đã lưu Quiz Blueprint “${created.title}”.` })
    } catch (error) {
      setMessage({ tone: 'danger', text: error instanceof Error ? error.message : 'Không lưu được Quiz Blueprint.' })
    } finally {
      setBlueprintSaveBusy(false)
    }
  }
  ...
  return <PageRoot className="page-stack bank-multipage bank-contract-page bank-quiz-page quiz-creation-workbench">
    ...
    <div className="bank-quiz-main-grid">
      ...
      >
        {!autoMap ? <div className="bank-quiz-empty-map">
          ...
          {autoMap.course_tree ? <ContentNotice tone={autoMap.course_tree.source === 'unavailable' ? 'danger' : autoMap.course_tree.source === 'cached' ? 'warning' : 'info'}><b>Nguồn cây Course CMS:</b> {autoMap.course_tree.source === 'direct' ? 'Open edX trực tiếp' : autoMap.course_tree.source === 'cached' ? `Dữ liệu sync cũ (${autoMap.course_tree.cached_block_count || 0} block)` : `Không khả dụng${autoMap.course_tree.error_code ? ` · ${autoMap.course_tree.error_code}` : ''}`}</ContentNotice> : null}
          {autoMap.blocking_errors?.length ? <ContentNotice tone="danger"><b>Chưa thể lưu hoặc tạo</b><ul>{autoMap.blocking_errors.slice(0, 5).map((item, index) => <li key={index}>{item}</li>)}</ul></ContentNotice> : null}
    ...
    >
      {createModal ? <>
          ...
          {(quizDifficultyTotal !== 100 || finalDifficultyTotal !== 100) ? <ContentNotice tone="warning">Tổng tỷ lệ Dễ/Trung bình/Khó của mỗi loại phải bằng 100%.</ContentNotice> : null}
          {createModal.kind === 'one' ? <div className="quiz-plan-preview-panel">
            {createModal.item.action === 'final_test' ? <ContentNotice tone="info"><b>Final test dùng bộ câu hỏi tổng hợp.</b> Hệ thống sẽ kiểm tra khả năng đáp ứng trên toàn bộ {(createModal.item as any).source_release_ids?.length || 0} Release nguồn ngay trước khi tạo và chặn nếu thiếu câu.</ContentNotice> : <><button className="btn secondary" type="button" disabled={planPreviewBusy || busy || Boolean(creatingKey)} onClick={previewCurrentPlan}>{planPreviewBusy ? 'Đang kiểm tra...' : 'Kiểm tra khả năng đáp ứng'}</button>
            {planPreview ? <><ContentNotice tone="success"><b>Bộ đề đủ câu để tạo Quiz.</b> Sinh viên sẽ làm {planPreview.total_questions} câu, được chọn từ {planPreview.assigned_question_count} câu khả dụng.</ContentNotice>{planPreview.warnings?.length ? <ContentNotice tone="info"><b>Điều chỉnh khi tạo đề</b><ul>{planPreview.warnings.map((warning, index) => <li key={`${warning}-${index}`}>{warning}</li>)}</ul></ContentNotice> : null}<div className="quiz-matrix-preview"><table><thead><tr><th>Độ khó</th><th>Số câu</th></tr></thead><tbody>{Object.entries(planPreview.matrix_target_counts || {}).filter(([, value]) => Number(value) > 0).map(([key, value]) => <tr key={key}><td>{DIFFICULTY_LABELS[key.replace(':auto', '').toUpperCase()] || key}</td><td>{value}</td></tr>)}</tbody></table></div></> : null}</>}
          </div> : null}
