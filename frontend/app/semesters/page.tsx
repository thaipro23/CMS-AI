...
function normalizeDateInput(value: string) {
  ...
  if (vn) {
    const dd = Number(vn[1])
    const mm = Number(vn[2])
    const yyyy = Number(vn[3])
...
export default function SemestersPage() {
  ...
  const save = async () => {
    ...
    const invalidDateBlock = form.blocks.find((block) => (block.start_date && !normalizeDateInput(block.start_date)) || (block.end_date && !normalizeDateInput(block.end_date)))
    if (invalidDateBlock) { setMessage('Ngày block phải nhập theo định dạng dd/mm/yyyy, ví dụ 11/05/2026'); return }
    setSaving(true); setMessage('')
    try {
