...
function statusClass(status?: string | null) {
  ...
  if (["mapped", "mapped_multiple", "already_mapped", "auto_mapped"].includes(value))
...
function StudentManagementSubjectsContent({ platform }: { platform: TrainingPlatform }) {
  ...
  const columns = useMemo<EnterpriseTableColumn<AcademicSubjectManagement>[]>(() => {
    ...
    const actions: EnterpriseTableColumn<AcademicSubjectManagement> = {
      ...
      render: (subject) => <div className="row-actions">
        ...
        {isCms && !["mapped", "mapped_multiple", "already_mapped", "auto_mapped"].includes(String(subject.course_mapping_status || "").toLowerCase()) ? <button className="btn small secondary" type="button" disabled={mappingSubjectId === subject.id} onClick={() => runAutoMap(subject)}>{mappingSubjectId === subject.id ? "Đang ghép..." : "Tự động ghép"}</button> : null}
      </div>,
    };
    if (isCms) return [
      ...common,
      ...
      { key: "course", header: "Course CMS", kind: "status", minWidth: 210, priority: "optional", hideable: true, render: (subject) => { const orgs = subject.openedx_orgs?.length ? subject.openedx_orgs : (subject.openedx_org ? [subject.openedx_org] : []); const ids = subject.openedx_course_ids?.length ? subject.openedx_course_ids : (subject.openedx_course_id ? [subject.openedx_course_id] : []); return <><span className={statusClass(subject.course_mapping_status)}>{subject.course_mapping_label || subject.course_mapping_status}</span><small>{orgs.length ? `Org ${orgs.join(", ")} · ` : ""}{ids.length > 1 ? `${ids.length} Course` : (ids[0] || subject.suggested_openedx_course_id || "N/A")}</small></>; } },
  ...
  return (
    <PageRoot className="page-stack enterprise-standard-page student-management-page academic-flow-page ux-enterprise-page">
      ...
      {!isCms && canImportUdemy ? <UdemyProgressImportDialog
        open={bulkUdemyImportOpen}
        branch={branch === 'ptcd' ? 'ptcd' : 'poly'}
        termId={termId}
        blockId=""
        delivery={null}
        headers={headers}
        onClose={() => { setBulkUdemyImportOpen(false); void loadSubjects(); }}
        onQueued={(response) => {
          setMessage(noticeInfo(response.message, 'Đã tạo tác vụ import hàng loạt'));
        }}
      /> : null}
    </PageRoot>
  );
