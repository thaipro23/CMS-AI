...
export default function SubjectManagementPage() {
  ...
  const loadDeliveries = useCallback(async () => {
    ...
    try {
      ...
      setSelected((current) => {
        ...
        current.forEach((id) => {
          if (response.items.some((item) => item.id === id)) next.add(id)
        })
        return next
      })
    } catch (err) {
  ...
  const bulkChangePlatform = async (platform: AcademicLearningPlatform) => {
    ...
    const allIds = rows.flatMap((item) => item.delivery_ids?.length ? item.delivery_ids : [item.id])
    const ids = allIds.filter((id, index) => allIds.indexOf(id) === index)
