...
export default function PremisesPage() {
  ...
  const remove = async (item: AcademicCampus) => {
    if (!window.confirm(`Xóa cơ sở ${codeLabel(item.campus_code)} - ${item.campus_name || ''}?\n\nCơ sở sẽ bị ẩn khỏi dropdown đồng bộ AP để tránh bấm nhầm.`)) return
    setSaving(true); setMessage('')
    try {
      await deleteAcademicCampus(jsonHeaders, item.id)
      setMessage(`Đã xóa cơ sở ${codeLabel(item.campus_code)}`)
      await load()
