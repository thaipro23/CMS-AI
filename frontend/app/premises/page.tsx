...
export default function PremisesPage() {
  ...
  const load = async (clearMessage = true) => {
    ...
    } catch (error) { notify(error instanceof Error ? error.message : 'Không tải được danh sách cơ sở') }
  ...
  const openCreate = () => { setMessage(null); setForm({ ...EMPTY_FORM, branch }); setEditingId(null); setModalOpen(true) }
  const edit = (item: AcademicCampus) => {
    setMessage(null); setEditingId(item.id)
  ...
  const save = async () => {
    ...
    if (!code) { notify('Thiếu mã cơ sở'); return }
    if (!name) { notify('Thiếu tên cơ sở'); return }
    ...
    try {
      ...
      notify(`Đã lưu cơ sở ${codeLabel(saved.campus_code)} · ${saved.campus_name}`, 'success')
      ...
      await load(false)
    } catch (error) { notify(error instanceof Error ? error.message : 'Lưu cơ sở thất bại') }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      notify(`Đã xóa cơ sở ${codeLabel(deleteTarget.campus_code)}`, 'success')
      setDeleteTarget(null)
      await load(false)
    } catch (error) { notify(error instanceof Error ? error.message : 'Xóa cơ sở thất bại') }
    finally { setSaving(false) }
  }
  ...
  if (!canManageCampusCatalog) return <PageRoot className="page-stack enterprise-standard-page training-operations-page premises-page"><EnterpriseScreenHeader eyebrow="Danh mục" title="Cơ sở" description="Danh mục cơ sở được nhập và chỉnh sửa thủ công tại đây." icon="campus" tone="blue" breadcrumbs={[{ label: 'Danh mục' }, { label: 'Cơ sở' }]} /><section className="card empty-state">Bạn không có quyền quản lý danh mục cơ sở.</section></PageRoot>

  return <PageRoot className="page-stack enterprise-standard-page training-operations-page premises-page">
    <EnterpriseScreenHeader eyebrow="Danh mục" title="Cơ sở" description="Nhập và chỉnh sửa thủ công danh mục cơ sở dùng cho đồng bộ AP và phạm vi vận hành đào tạo." icon="campus" tone="blue" breadcrumbs={[{ label: 'Danh mục' }, { label: 'Cơ sở' }]} secondaryActions={<button className="btn secondary" type="button" disabled={loading} onClick={() => void load()}>Làm mới</button>} primaryAction={<button className="btn" type="button" onClick={openCreate}>Thêm cơ sở thủ công</button>} />
    ...
    >
      <InlineNotice notice={message} />
      {deleteTarget ? <div className="academic-confirm-body"><p>Cơ sở <b>{codeLabel(deleteTarget.campus_code)} · {deleteTarget.campus_name || 'Không có tên'}</b> sẽ ngừng sử dụng trong danh mục cơ sở của hệ thống.</p><div className="academic-confirm-summary"><span>Hệ</span><b>{branchLabel(deleteTarget.branch)}</b><span>Mã cơ sở</span><b>{codeLabel(deleteTarget.campus_code)}</b></div></div> : null}
    </AccessibleDialog>
  </PageRoot>
}
