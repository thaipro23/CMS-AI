...
export default function AnalyticsLearningPage() {
  ...
  const subjectColumns = useMemo<EnterpriseTableColumn<AcademicSubjectManagement>[]>(() => [
    ...
    { key: 'course', header: 'Course CMS', kind: 'status', minWidth: 180, priority: 'important', hideable: true, render: (item) => { const count = item.openedx_course_ids?.length || (item.openedx_course_id ? 1 : 0); return <span className={count ? 'status-pill success' : 'status-pill warning'}>{count ? (count > 1 ? `Đã ghép ${count} Course` : 'Đã ghép') : 'Chưa ghép'}</span>; } },
  ...
  const classColumns = useMemo<EnterpriseTableColumn<AnalyticsClassBehaviorOverviewItem>[]>(() => [
    ...
    { key: 'course', header: 'Course CMS', kind: 'status', minWidth: 210, priority: 'optional', defaultVisible: false, hideable: true, render: (item) => item.openedx_course_id ? <><span className="status-pill success">Đã ghép · {item.openedx_org || 'ORG'}</span><small className="enterprise-clamp-1">{item.openedx_course_id}</small></> : <span className="status-pill warning">Chưa ghép</span> },
