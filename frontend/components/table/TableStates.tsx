...
export function TableLoadingState({ label = 'Đang tải dữ liệu...' }: { label?: string }) {
  return <div className="enterprise-table-state loading visual-state" role="status" aria-live="polite"><VisualIcon label="Đang tải dữ liệu" icon="database" tone="blue" /><span className="spinner" aria-hidden="true" /><div><b>{label}</b><small>Vui lòng chờ, dữ liệu hiện có sẽ không bị thay đổi.</small></div></div>
}
...
export function TableErrorState({ title = 'Không thể tải dữ liệu', message, code, onRetry }: { title?: string; message?: string; code?: string; onRetry?: () => void }) {
  return <div className="enterprise-table-state error visual-state" role="alert"><VisualIcon label="Có lỗi" icon="alert" tone="red" /><div><b>{title}</b>{message && <small>{message}</small>}{code && <code>{code}</code>}</div>{onRetry && <button className="btn small secondary" type="button" onClick={onRetry}>Thử lại</button>}</div>
}
