...
export async function publishBankRelease(
  ...
  return enqueueAndWait<BankReleasePublishResult>(
    headers,
    queued,
    20 * 60 * 1000,
  );
...
export async function createAcademicTrainingTeacherCacheJob(
  headers: HeadersInit,
  filters: { termId: string; campus?: string; branch?: string },
...
export async function syncAcademicCampusesFromAp(
  headers: HeadersInit,
  branch = "poly",
): Promise<AcademicCampus[]> {
  const params = new URLSearchParams();
  ...
  return parseResponse(
    await apiFetch(
      `${API}/academic/campuses/sync-from-ap?${params.toString()}`,
      { method: "POST", headers },
    ),
  );
}

export async function seedAcademicCampusesFromEnv(
  headers: HeadersInit,
  branch = "poly",
): Promise<AcademicCampus[]> {
  const params = new URLSearchParams();
  ...
  return parseResponse(
    await apiFetch(
      `${API}/academic/campuses/seed-from-env?${params.toString()}`,
      { method: "POST", headers },
    ),
  );
}
