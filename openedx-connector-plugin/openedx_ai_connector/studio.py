...
from .auth import (
    _NoRedirectHandler,
    _connector_hmac_secret,
    _env_bool,
    _json_response,
    _read_json_body,
    _require_connector_admin,
    _require_connector_write,
    _same_request_host,
    _setting_or_env,
...
MAX_AI_CONNECTOR_BODY_BYTES = int(os.environ.get('AI_CONNECTOR_MAX_BODY_BYTES') or 24 * 1024 * 1024)
...
def create_quiz_node(request, course_id: str):
    ...
    unit_title = _normalize_xblock_title(payload.get('unit_title'), 'Quiz')
    metadata = payload.get('metadata') or {}
    sequential_fields = {
        'format': metadata.get('format') or metadata.get('grade_as') or 'Quiz',
        'graded': True if metadata.get('graded') is None else bool(metadata.get('graded')),
    }
...
_QUESTION_MEDIA_ALLOWED_MIME = {'image/png', 'image/jpeg', 'image/webp'}
_QUESTION_MEDIA_MAX_BYTES = 4 * 1024 * 1024
_QUESTION_MEDIA_MAX_TOTAL_BYTES = 16 * 1024 * 1024
_QUESTION_MEDIA_MAX_ITEMS = 4
_QUESTION_MEDIA_PATH_RE = re.compile(r'^[A-Za-z0-9_.-]+(?:/[A-Za-z0-9_.-]+)*$')


def _decode_question_media_assets(raw_assets: object) -> list[dict[str, Any]]:
    if raw_assets in (None, []):
        return []
    if not isinstance(raw_assets, list) or len(raw_assets) > _QUESTION_MEDIA_MAX_ITEMS:
    ...
    for item in raw_assets:
        ...
        if not placeholder.startswith('__ACMS_MEDIA_') or not placeholder.endswith('__') or placeholder in seen_placeholders:
            raise ValueError('Media placeholder không hợp lệ hoặc bị trùng.')
        if not path or '..' in path.split('/') or not _QUESTION_MEDIA_PATH_RE.fullmatch(path) or path in seen_paths:
            raise ValueError('Media file_path không hợp lệ hoặc bị trùng.')
        if mime not in _QUESTION_MEDIA_ALLOWED_MIME:
            raise ValueError('Media MIME không được hỗ trợ.')
        try:
            content = base64.b64decode(encoded, validate=True)
        except Exception as exc:
            raise ValueError('Media base64 không hợp lệ.') from exc
        if not content or len(content) > _QUESTION_MEDIA_MAX_BYTES:
        ...
        total += len(content)
        if total > _QUESTION_MEDIA_MAX_TOTAL_BYTES:
...
def _import_problem_olx_v2(request, course_id: str, library_key: str, display_name: str, olx: str, metadata: dict | None = None, tag_names: list | None = None, assets: list | None = None) -> dict:
    ...
    # Upload question images as static assets owned by this Library block, then
    # resolve ACMS placeholders. MinIO/presigned URLs are never embedded in OLX.
    olx, uploaded_assets = _upload_question_media_assets(usage_key, olx, assets or [], user)
...
def import_problem_to_library(request, library_key: str):
    ...
    if request.method != 'POST':
        return HttpResponseBadRequest('POST required')
    payload, body_error = _read_json_body(request)
    if body_error:
        return body_error
    payload = payload or {}
