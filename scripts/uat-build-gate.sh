...
EXPECTED_ALEMBIC_HEAD='0062_v25_9_16_7_2_64_40'
if command -v alembic >/dev/null 2>&1; then
  ...
  record_status FAIL ALEMBIC_CLI_MISSING "alembic CLI is required to verify the migration graph"
fi
...
run_and_record MIGRATION_COMPILE "python -m compileall backend/alembic/versions" migration_compile.log python -m compileall -q backend/alembic/versions
run_and_record CONNECTOR_COMPILE "python -m compileall openedx connector" connector_compile.log python -m compileall -q openedx-connector-plugin/openedx_ai_connector
run_and_record ERROR_BOUNDARY_CONTRACT "static API/DB/worker exception boundary gate" error-boundary-contract.log python scripts/error-boundary-contract-check.py --root . --report "$OUT_DIR/error-boundary-contract.md"
...
# Frontend typecheck/build. v25.9.16.7.2.64.16.5.7.2.18 delegates the deep
...
if [[ "$RUN_FRONTEND_BUILD" == "1" ]]; then
  if OUT_DIR="$OUT_DIR/frontend-build-verify" EXPECTED_VERSION="$EXPECTED_VERSION" RUN_NPM_CI="$RUN_FRONTEND_INSTALL" RUN_FRONTEND_BUILD=1 bash ./scripts/frontend-build-verify.sh > "$OUT_DIR/frontend-build-verify.log" 2>&1; then
...
if [[ "$RUN_REVIEW_PACK" == "1" ]]; then
  if OUT_DIR="$OUT_DIR/claude-code-review-pack" EXPECTED_VERSION="$EXPECTED_VERSION" bash ./scripts/claude-code-review-pack.sh > "$OUT_DIR/claude-code-review-pack.log" 2>&1; then
    record_status PASS CLAUDE_REVIEW_PACK "claude-code-review-pack.sh passed" "claude-code-review-pack/review-summary.json"
  else
    record_status FAIL CLAUDE_REVIEW_PACK "claude-code-review-pack.sh reported FAIL status" "claude-code-review-pack.log"
  fi
else
