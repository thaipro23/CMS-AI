...
if command -v alembic >/dev/null 2>&1; then
  ...
  if [[ "$HEAD_COUNT" == "1" ]] && grep -q "$EXPECTED_ALEMBIC_HEAD" "$OUT_DIR/alembic-heads.txt"; then
    record_status PASS ALEMBIC_HEAD_KNOWN "Single Alembic head is $EXPECTED_ALEMBIC_HEAD"
  else
    record_status FAIL ALEMBIC_HEAD_KNOWN "Alembic graph must have exactly one expected head: $EXPECTED_ALEMBIC_HEAD" "alembic-heads.txt"
  fi
else
  record_status WARN ALEMBIC_CLI_MISSING "alembic CLI unavailable; migration graph not verified in this review pack"
fi

# 3b) Exception/error boundary contract. Broad best-effort cleanup is reported,
# but swallowed Celery failures / bare except / raw broad-error leaks are blockers.
if python scripts/error-boundary-contract-check.py --root . --report "$OUT_DIR/error-boundary-contract.md" > "$OUT_DIR/error-boundary-contract.log" 2>&1; then
  record_status PASS ERROR_BOUNDARY_CONTRACT "No blocker exception-boundary patterns detected" "error-boundary-contract.md"
else
  record_status FAIL ERROR_BOUNDARY_CONTRACT "Dangerous exception-boundary pattern detected" "error-boundary-contract.md"
fi
...
if bash ./scripts/cors-request-id-preflight-report.sh "$OUT_DIR/cors-request-id-preflight" > "$OUT_DIR/cors-request-id-preflight.log" 2>&1; then
...
if bash ./scripts/production-security-closure-report.sh "$OUT_DIR/production-security-closure" > "$OUT_DIR/production-security-closure.log" 2>&1; then
...
if bash ./scripts/performance-worker-reliability-report.sh "$OUT_DIR/performance-worker-reliability" > "$OUT_DIR/performance-worker-reliability.log" 2>&1; then
...
if bash ./scripts/npm-public-registry-lockfile-report.sh "$OUT_DIR/npm-public-registry" > "$OUT_DIR/npm-public-registry.log" 2>&1; then
  record_status PASS NPM_PUBLIC_REGISTRY "npm lockfiles use registry.npmjs.org only"
else
  record_status FAIL NPM_PUBLIC_REGISTRY "npm lockfile contains non-public registry URLs; see npm-public-registry.log"
fi

# Frontend runtime, modal and route-state contracts.
if bash ./scripts/frontend-runtime-contracts-report.sh "$OUT_DIR/frontend-runtime-contracts" > "$OUT_DIR/frontend-runtime-contracts.log" 2>&1; then
...
if bash ./scripts/ci-e2e-container-hardening-report.sh "$OUT_DIR/ci-e2e-container-hardening" > "$OUT_DIR/ci-e2e-container-hardening.log" 2>&1; then
...
if bash ./scripts/backend-runtime-name-audit.sh "$OUT_DIR/backend-runtime-name-audit" > "$OUT_DIR/backend-runtime-name-audit.log" 2>&1; then
...
if bash ./scripts/frontend-layout-integrity-report.sh "$OUT_DIR/frontend-layout-integrity" > "$OUT_DIR/frontend-layout-integrity.log" 2>&1; then
...
if [[ "$INCLUDE_BUILD_GATE" == "1" ]]; then
  if OUT_DIR="$OUT_DIR/uat-build-gate" EXPECTED_VERSION="$EXPECTED_VERSION" STRICT="$STRICT_BUILD_GATE" bash ./scripts/uat-build-gate.sh > "$OUT_DIR/uat-build-gate.log" 2>&1; then
...
if bash ./scripts/uat-http-env-compatibility-report.sh > "$OUT_DIR/uat-http-env-compatibility.log" 2>&1; then
...
if bash ./scripts/uat-build-backend-health-hotfix-report.sh > "$OUT_DIR/uat-build-backend-health-hotfix.log" 2>&1; then
...
Review target: AI Server / Open edX CMS v25.9.16.7.2.64.16.5.7.2.18 — Question Types, Open edX Import, Quiz Type Quota & Error Boundary Hardening.
...
This version extends the Question Bank end-to-end. Reviewers should inspect canonical question content/media, AI single/multi generation, Open edX OLX import/export, exact Quiz Blueprint question-type quota, connector static-asset publishing, worker failure durability, and the API/DB/error boundary mapping while confirming existing AP sync, security, performance, RBAC and analytics gates remain intact.
...
- `frontend-typecheck-required.txt`: command to run the current .18 frontend build verifier.
