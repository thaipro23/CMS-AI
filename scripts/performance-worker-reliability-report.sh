...
teacher = (root / 'frontend/app/teacher-management/TeacherManagementPlatformPage.tsx').read_text(encoding='utf-8')
...
checks = [
    ...
    check('SERVER_SIDE_SCOPE_SEARCH', 'searchSubjects' in users and 'searchSubjectOfferings' in users and 'getSubjectChapters(headers)' not in users and "q: str | None = Query(None, max_length=120)" in bank_route, 'RBAC catalog uses bounded server-side search instead of eager full hierarchy loading.'),
]
