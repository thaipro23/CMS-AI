...
check 'Production env configures canonical AP get-course catalog/cache' bash -lc "grep -q '^ACADEMIC_AP_GET_COURSE_ENDPOINT=/get-course$' .env.production.example && grep -q '^ACADEMIC_AP_GET_COURSE_FILE_CACHE_ENABLED=' .env.production.example && grep -q '^ACADEMIC_AP_GET_COURSE_FILE_CACHE_DIR=/tmp/ai-server-ap-cache/get-course$' .env.production.example && grep -q '^ACADEMIC_AP_GET_COURSE_FILE_CACHE_TTL_SECONDS=' .env.production.example && grep -q '^ACADEMIC_AP_GET_COURSE_FILE_CACHE_REFRESH=' .env.production.example"
