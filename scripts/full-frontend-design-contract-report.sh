...
STUDENT="$ROOT/frontend/app/student-management/StudentManagementPlatformPage.tsx"
...
RBAC_SCHEMA="$ROOT/backend/app/schemas/rbac.py"
RBAC_SERVICE="$ROOT/backend/app/services/business_rbac.py"
RBAC_SERVICE="$ROOT/backend/app/services/business_rbac.py"
DOCKERFILE="$ROOT/frontend/Dockerfile"

has_chapter_action_counts() {
  grep -Eq 'Tạo câu hỏi \(\$\{usedQuestionCount\}/\$\{chapterQuestionLimit\}\)' "$CHAPTER" \
    && grep -Fq 'Duyệt câu hỏi (${pendingReviewCount} câu chờ duyệt)' "$CHAPTER"
}
has_student_subject_automap() {
  grep -Fq 'title={`Danh sách môn ${platformLabel}`}' "$STUDENT" \
    && grep -Fq 'Tự động ghép Course CMS' "$STUDENT"
}
...
check 'Chapter action labels carry useful counts' has_chapter_action_counts
...
check 'Student auto-map belongs to subject section' has_student_subject_automap
...
check 'Legacy CAMPUS_MANAGER cannot be granted anew' bash -c "grep -Fq \"!['CAMPUS_MANAGER', 'TEACHER_ASSIGNED'].includes(role.code)\" '$USERS' && grep -Fq \"value == 'CAMPUS_MANAGER'\" '$RBAC_SCHEMA' && grep -Fq \"role_code == CAMPUS_MANAGER\" '$RBAC_SERVICE'"
